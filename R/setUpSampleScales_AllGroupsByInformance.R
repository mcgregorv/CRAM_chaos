#
## vary initial conditions of all species groups, using informance to influence how much they can vary
# sample from normal distirbution for each tracer, for each run - 361 tracers, 35 runs
...
version<- "" # there is another 'version' later defined, this one is not really used. was thinking i might wnt it as a subversion
...
nruns <- 35; sampleVersion<-"InformanceAll"
...
for(i in 1:nvars){
  ...
  if(!(thisVar %in% doneVars)){
    ...
    if(length(asTest)==0){
      ...
      thisInf <- rankTable$informPerformRank[rankTable$Code==thisCode]
      thisSD<-ageDevs[match(thisInf, infColIndex)]
