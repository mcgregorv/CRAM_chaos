...
par(mfrow=c(5,4), mar=c(3.5,4,1.5,4))
for(g in 1:this_ng){
  thisCode <- BPgroups[g]
  ...
  par(las=1)
  plot(x=allTime[1:nts],y=thisBaseTracer, ylim=c(0,thisMax*1.1), type="n", ylab="Biomass (tonnes)", xlab="", cex=0.5, cex.axis=axisCex)
  ...
  par(new=TRUE)
  par(las=1)
  plot(x=allTime, y=thisFishCV, ylim=c(0, 1), pch=4, col=myBlue, cex=0.2, ylab="", xlab="", yaxt="n", xaxt="n")
  points(x=allTime, y=thisNotFishedCV, pch=3, col=myOrange,cex=0.2)
  axis(at=thisYaxis, labels = 100*thisYaxis, side=4, cex.lab=axisCex, cex.axis=axisCex); 
  par(las=0)
  mtext("CV (%)", side=4, adj=0.5, line=1.5, cex=0.5)
}
makeBlankPlot()
par(lend=1)
legend(legend=c("Fished models","Not fished models", "Fished CVs", "Not fished CVs"), x="center", col=c(myBlue, myOrange, myBlue, myOrange), 
       pch=c(NA, NA, 4, 3), lty=c(1,1,NA, NA), lwd=c(2,2,1,1), bty="n")
dev.off()

axisCex<-0.8
ASgroups <- thisGroupsDF$Code[!BPindex]; this_ng <- length(ASgroups)
pdf(paste(plotPath,"AgeStructuredTracersALL_p1.pdf", sep=""), height=12, width=10)
par(mfrow=c(6,4), mar=c(3.5,4,1.5,4))
for(g in 1:23){
  thisCode <- ASgroups[g]
  gIndex <- grep(thisCode, thisGroupsDF$Code);
  if(length(gIndex>1)){gIndex <- gIndex[thisGroupsDF$Code[gIndex]==thisCode]}
...
par(mfrow=c(4,4), mar=c(3.5,4,1.5,4))
for(g in 24:this_ng){
  thisCode <- ASgroups[g]
  gIndex <- grep(thisCode, thisGroupsDF$Code);
  if(length(gIndex>1)){gIndex <- gIndex[thisGroupsDF$Code[gIndex]==thisCode]}
...
axisCex=1
pdf(paste(plotPath, "CVsByTimeExamples.pdf", sep=""), height=4, width=15)
par(mfrow=c(1,2), mar=c(4,6,1.5,5), oma=c(0,0,0,12))
thisCode <- "DR"
gIndex <- grep(thisCode, thisGroupsDF$Code);
if(length(gIndex>1)){gIndex <- gIndex[thisGroupsDF$Code[gIndex]==thisCode]}
thisName<- gsub("_", " ", str_trim(thisGroupsDF$Name[gIndex]))
...
par(new=TRUE)
par(las=1)
plot(x=allTime, y=thisFishCV, ylim=c(0, 1), pch=4, col=myBlue, cex=0.5, ylab="", xlab="", yaxt="n", xaxt="n")
points(x=allTime, y=thisNotFishedCV, pch=3, col=myOrange,cex=0.5)
axis(at=thisYaxis, labels = 100*thisYaxis, side=4, cex.lab=axisCex, cex.axis=axisCex); 
par(las=0)
...
par(xpd="NA")
par(lend=1)
legend(legend=c("Fished models","Not fished models", "Fished CVs", "Not fished CVs"), x="right", inset=-0.75, col=c(myBlue, myOrange, myBlue, myOrange), 
       pch=c(NA, NA, 4, 3), lty=c(1,1,NA, NA), lwd=c(2,2,1,1), bty="n")
dev.off()
...
ALLtest<-c(unlist(storeAllBPModelDFs), unlist(storeUnfishedModelDFs), unlist(storeFishedModelDFs))
...
pdf(paste(plotPath, "FactorsByTimeAndR2.pdf", sep=""), height=4, width=20)
plot_grid(bpA, bpB, bpC, nrow=1)
dev.off()


pdf(paste(plotPath, "FactorNumberByTimeAndR2.pdf", sep=""), height=4, width=20)
plot_grid(bpFnumsA, bpFnumsB, bpFnumsC, nrow=1)
dev.off()


pdf(paste(plotPath, "FactorAndNumberByTimeAndR2.pdf", sep=""), height=6, width=16)
...
pdf(paste(plotPath, "BPFactorsByTimeAndR2.pdf", sep=""), height=4, width=20)

plot_grid(bpA, bpB, bpC, nrow=1)
dev.off()


pdf(paste(plotPath, "BPFactorNumberByTimeAndR2.pdf", sep=""), height=4, width=15)
plot_grid(bpFnumsA, bpFnumsB, bpFnumsC, nrow=1)
dev.off()


pdf(paste(plotPath, "BPFactorAndNumberByTimeAndR2.pdf", sep=""), height=5, width=15)
plot_grid(bpA, bpB, bpC, bpFnumsA, bpFnumsB, bpFnumsC, nrow=2)
dev.off()
...
pdf(paste(plotPath, "ASFactorsByTimeAndR2.pdf", sep=""), height=4, width=20)
plot_grid(bpA, bpB, bpC, nrow=1)
dev.off()
...
thisDescriptions <- c("ChaosAlt", "Trophic level",  "Level 1 connections", "Lifespan (years)", "", "Diet proportion by top prey")
...
pdf(paste(plotPath,"residualsALL.pdf", sep=""), height=3.5, width=15)
par(mfrow=c(1,3), mar=c(4.5,5,1.2,1))
...
pdf(paste(plotPath,"residualsBPall.pdf", sep=""), height=7, width=15)
par(mfrow=c(2,3), mar=c(4.5,5,1.2,1))
...
mtext("C", side=3, adj=0, font=2)
# by NumL1Cons
plot(x=BPmeltData$NumL1cons, y=BP_pearsonsResids, pch=20, col=myBlue_trans, ylab="Pearson's residuals", xlab="Number of level 1 connections", cex.lab=1.5)
...
pdf(paste(plotPath,"residualsASall.pdf", sep=""), height=3.5, width=20)
par(mfrow=c(1,4), mar=c(4.5,5,1.2,1))
...
mtext("C", side=3, adj=0, font=2)
# by NumL1cons
plot(x=ASmeltData$NumL1cons, y=AS_pearsonsResids, pch=20, col=myBlue_trans, ylab="Pearson's residuals", xlab="Number of level 1 trophic connections", cex.lab=1.5)
mtext("D", side=3, adj=0, font=2)
dev.off()
...
pdf(paste(plotPath,"residualsASFished.pdf", sep=""), height=3.5, width=20)
par(mfrow=c(1,4), mar=c(4.5,5,1.5,1))
...
pdf(paste(plotPath,"residualsASUnfished.pdf", sep=""), height=7, width=15)
par(mfrow=c(2,3), mar=c(4.5,5,1.5,1))
...
mtext("C", side=3, adj=0, font=2)
# by NumL1cons
plot(x=ASUnfishedMeltData$NumL1cons, y=ASUnfished_pearsonsResids, pch=20, col=myBlue_trans, ylab="Pearson's residuals", xlab="Number of level 1 trophic connections", cex.lab=1.5)
mtext("D", side=3, adj=0, font=2)
# by Informance
...
axis(at= names(temp) , labels=names(temp), side=1)
...
## TL:ChaosALt
pdf(paste(plotPath,"GLMeffectsAllBpAS.pdf", sep=""), height=3.5, width=15)
par(mfrow=c(1,3), mar=c(4.5,5.5,1.5,1))
plotIntEffect_noChaosRun(thisIntVar, this_model=ALLmeltModel, thisMin=0, inputMax=1, updateMax=TRUE, thisData=ALLmeltData)
mtext("A: ALL species groups", side=3, adj=0)
plotIntEffect_noChaosRun(thisIntVar, this_model=BPmeltModel, thisMin=0, inputMax=1, updateMax=TRUE, thisData=BPmeltData)
mtext("B: BP species groups", side=3, adj=0)
plotIntEffect_noChaosRun(thisIntVar, this_model=ASmeltModel, thisMin=0, inputMax=1, updateMax=TRUE, thisData=ASmeltData)
mtext("C: AS species groups", side=3, adj=0)
dev.off()

# also do NumL1cons:B0 for BP, and chaosAlt:NumL1cons for AS model
pdf(paste(plotPath,"GLMeffectsBpAS.pdf", sep=""), height=3.5, width=10)
par(mfrow=c(1,2), mar=c(4.5,5.5,1.5,1))
plotIntEffect_noChaosRun("NumL1cons:B0", this_model=ALLmeltModel, thisMin=0, inputMax=1, updateMax=TRUE, thisData=ALLmeltData)
mtext("A: BP species groups", side=3, adj=0)
plotIntEffect_noChaosRun("ChaosAlt:NumL1cons", this_model=BPmeltModel, thisMin=0, inputMax=1, updateMax=TRUE, thisData=BPmeltData)
mtext("B: AS species groups", side=3, adj=0)
dev.off()
...
for(i in 1:nivars){
  ...
  plotIntEffect_noChaosRun(thisIntVar = intVars[i], this_model=ASAllModel, inputMax=0.3, updateMax = FALSE, thisData = testMelt[thisASindex,])
