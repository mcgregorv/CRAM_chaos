...
asIndex <- groupsDF$NumCohorts>1; as_codes <- as.character(groupsDF$Code)[asIndex]
bpData2fit <- allData2fit[!(allData2fit$Code %in% as_codes),]


hist(bpData2fit$RelBiomass)
...
fitData$logRel <- log(fitData$RelBiomass +1, base=exp(1))
...
for(i in 1:nivars){
  ...
  thisMin <- 0
  plotIntEffect_noChaosRun(thisIntVar = thisIntVar, this_model=this_model, inputMax=0.1, updateMax = TRUE, thisData=fitData)
...
# save(this_model, file=paste(DIR$'Data', "Chaos\\BP_glmModel", sep=""))
