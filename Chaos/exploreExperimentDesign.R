...
## take out any vars that have a matching 'Nums' var - in these the _N initial condition won't be used anyway
...
for(i in 1:nvars){
  ...
  if(!(thisVar %in% doneVars)){
    # test if age-structured
    asTest<-grep("_Nums", thisVar)
    if(length(asTest)==0){
      thisSD <- nageSdev
      allMyVarsIndex <- vars == thisVar
      ...
      thisSD<-ageDevs[match(thisInf, infGrades)]
